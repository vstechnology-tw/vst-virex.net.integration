...
這個儲存庫定義 Virex.NET 相容端點的公開整合介面。模擬器與正式產品應該暴露同一份合約，讓整合 Client 可以在不改變整合模型的前提下切換端點。
...
flowchart LR
    ...
    subgraph ClientSide["Client 端整合方式"]
...
SDK 是選用工具。廠商可以用 raw REST/MQTT/TCP 整合，也可以用 `Virex.NET.Client`，但兩種方式都必須遵守同一份公開合約。
...
| 套件或應用程式 | 角色 |
| --- | --- |
| `Virex.NET.Contracts` | 以 C# 模型提供共用公開資料模型、REST 路由常數、MQTT 主題名稱、TCP/NDJSON 解析器與事件格式化工具。 |
| `Virex.NET.Client` | 選用的 C# SDK wrapper，給需要強型別輔助工具的廠商使用。它不是必要的整合邊界。 |
...
| 傳輸方式 | 方向 | 職責 |
| --- | --- | --- |
| REST | Client 到服務 | 命令與查詢：狀態、ProductInfo、系統生命週期、執行、結果清單。 |
| TCP / NDJSON | 雙向 | 直接 socket 整合，用於命令資料框與事件資料框。 |
| MQTT | 服務到 Client | 只做傳出事件通知。MQTT 不用於命令。 |
...
- 可以使用 raw transport call 或選用 C# SDK，但不改變端點合約。
- 不依賴模擬器 UI 細節。
...
- 通訊資料模型與通訊協定常數。
- REST、MQTT、TCP/NDJSON 資料格式化與解析工具。
- C# SDK 包裝層。
