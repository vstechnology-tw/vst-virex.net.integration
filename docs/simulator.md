...
`Virex.NET.Simulator.WPF` is the local endpoint for integration development. From the perspective of RESTful API, TCP, MQTT, data models, state, and events, it should behave like a Virex.NET service compatible with production.
...
| --- | --- | --- |
| 1 | **Connection Settings** | Defines the endpoints exposed by the simulator. Use **RESTful API prefix** for the HTTP base address, **TCP port** for the NDJSON socket listener, **MQTT host** and **MQTT port/topic** for the embedded MQTT broker and topic prefix, and **Result prefix** only when result IDs or result paths need a test prefix. |
...
| 3 | **State** | Shows the current simulator state and provides the main interaction buttons. **Start Servers** opens the RESTful API, TCP, and MQTT endpoints. **Initialize**, **Deinitialize**, **Start Single**, **Start Continue**, and **Stop** drive the same public state transitions that an external client can call through RESTful API. |
...
| --- | --- |
| Contract verification | Confirm that the vendor application uses the same RESTful API routes, data models, TCP frames, and MQTT topics as production. |
...
| --- | --- |
| Initialize | RESTful API command returns `Ready`; state events are published. |
| ProductInfo update | RESTful API command returns `Ready`; ProductInfo events are published. |
| Start single | RESTful API command returns `Running`; state changes to `Running`; a result-created event is published; run-completed returns the state to `Ready`. |
| Start continue | RESTful API command returns `Running`; state stays `Running`; result-created events continue until a stop command is accepted. |
