...
| --- | --- | --- | --- |
| `initialize` | `type` を含むコマンドフレーム | `Uninitialized` | `Initializing` に遷移します。完了後、`Ready` の `statusChanged` を送信します。 |
| `deinitialize` | `type` を含むコマンドフレーム | `Ready` | `Deinitializing` に遷移します。完了後、`Uninitialized` の `statusChanged` を送信します。 |
