...
| --- | --- | --- | --- |
| `initialize` | `{"type":"initialize"}` | `Uninitialized` | `Initializing` に遷移します。完了後、`Ready` の `statusChanged` を送信します。 |
| `deinitialize` | `{"type":"deinitialize"}` | `Ready` | `Deinitializing` に遷移します。完了後、`Uninitialized` の `statusChanged` を送信します。 |
