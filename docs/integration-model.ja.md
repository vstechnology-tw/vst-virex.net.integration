...
SDK はオプションです。ベンダーは、Raw REST/MQTT/TCP 統合または `Virex.NET.Client` を使用できますが、どちらの方法も同じ公開コントラクトに準拠する必要があります。
...
|パッケージまたはアプリケーション |役割 |
| --- | --- |
| `Virex.NET.Contracts` |公開 C# データモデル、REST ルート定数、MQTT トピック名、TCP/NDJSON パーサー、およびイベントフォーマット用ヘルパーを提供します。 |
...
|通信方式 |方向 |責任 |
| --- | --- | --- |
| REST |クライアントからサービスへ |状態、ProductInfo、システムのライフサイクル、実行、結果リストのコマンドとクエリ。 |
...
- 通信データモデルとプロトコル定数。
- REST、MQTT、TCP/NDJSON データのフォーマットおよび解析ツール。
- C# SDK ラッパー層。
