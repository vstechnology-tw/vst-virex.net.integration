...
| TCP / NDJSON |双方向 |コマンド フレーム、クエリ フレーム、直接応答、およびイベント フレームのソケット直接統合。 |
| MQTT |双方向 |サービスからクライアントへのイベント通知に加え、`commands/...` topic のコマンド/クエリ要求と `responses/{correlationId}` topic の対応応答。 |
