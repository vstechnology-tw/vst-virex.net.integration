...
| Start | 시뮬레이터로 수신 | `{"type":"start","condition":"golden-sample","runMode":"continue"}` | `condition` is optional. `runMode` defaults to `continue`; legacy `{"type":"start"}` remains valid. |
| Stop | 시뮬레이터로 수신 | `{"type":"stop","reason":"operator-request"}` | `reason` is optional; legacy `{"type":"stop"}` remains valid. |
