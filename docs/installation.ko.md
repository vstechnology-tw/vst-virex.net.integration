...
시뮬레이터는 저장소 릴리스 페이지에 ZIP 파일로 릴리스됩니다.

[최신 시뮬레이터 버전 다운로드](https://github.com/vstechnology-tw/vst-virex.net.integration/releases/latest)

각 우편번호에는 `Virex.NET.Simulator.WPF.exe`가 포함되어 있습니다. 대상 환경에 따라 패키지를 선택하십시오:

| 파일 | 런타임 | 사용 시기 |
| --- | --- | --- |
| `Virex.NET.Simulator-vX.Y.Z-net48-win-x64.zip` | .NET Framework 4.8 | 대상 Windows 시스템에 이미 .NET Framework 4.8이 설치된 경우에 사용합니다. |
| `Virex.NET.Simulator-vX.Y.Z-net8.0-windows-win-x64-self-contained.zip` | .NET 8 윈도우 | 런타임을 포함하는 현재 Windows 빌드가 필요할 때 사용합니다. |
| `Virex.NET.Simulator-vX.Y.Z-net10.0-windows-win-x64-self-contained.zip` | .NET 10 윈도우 | 지원되는 최신 Windows 대상의 유효성을 검사해야 할 때 사용합니다. |
...
| 패키지 | 목적 |
| --- | --- |
| [Virex.NET.Contracts](https://github.com/vstechnology-tw/vst-virex.net.integration/releases/latest) | 공개 C# 데이터 모델, REST 경로, MQTT 토픽 이름, TCP/NDJSON 형식 지정 및 구문 분석 도구를 제공합니다. |
