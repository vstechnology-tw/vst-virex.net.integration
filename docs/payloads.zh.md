...
| 群組 | 模型 | 用途 |
| --- | --- | --- |
| System | [SystemStatus](payloads/system/system-status.md), [ErrorInfo](payloads/system/error-info.md) | 目前系統狀態與作用中錯誤資訊。 |
| Product | [ProductInfo](payloads/product/product-info.md) | 執行與結果關聯使用的產品資訊。 |
| Commands | [CommandResponse](payloads/commands/command-response.md), [SystemStartRequest](payloads/commands/system-start-request.md), [SystemStopRequest](payloads/commands/system-stop-request.md), [ControlRunModes](payloads/commands/control-run-modes.md) | 命令要求與回應資料。 |
| Results | [ResultSummary](payloads/results/result-summary.md), [ResultList](payloads/results/result-list.md) | 結果摘要資料與 REST 清單包裝。 |
