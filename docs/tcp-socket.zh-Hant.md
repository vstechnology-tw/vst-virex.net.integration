...
| 資料框類型 | 資料內容 | 合法狀態 | 結果 |
| --- | --- | --- | --- |
| `initialize` | 命令資料框加上 `type` | `Uninitialized` | 進入 `Initializing`；完成後送出狀態為 `Ready` 的 `statusChanged`。 |
| `deinitialize` | 命令資料框加上 `type` | `Ready` | 進入 `Deinitializing`；完成後送出狀態為 `Uninitialized` 的 `statusChanged`。 |
