...
シミュレーターは、リポジトリ リリース ページで ZIP ファイルとしてリリースされます。

[最新バージョンのシミュレーターをダウンロードする](https://github.com/vstechnology-tw/vst-virex.net.integration/releases/latest)

各 ZIP には `Virex.NET.Simulator.WPF.exe` が含まれています。ターゲット環境に応じてパッケージを選択してください。

|ファイル |ランタイム |いつ使用するか |
| --- | --- | --- |
| `Virex.NET.Simulator-vX.Y.Z-net48-win-x64.zip` | .NET Framework 4.8 |ターゲット Windows マシンに既に .NET Framework 4.8 がインストールされている場合に使用します。 |
| `Virex.NET.Simulator-vX.Y.Z-net8.0-windows-win-x64-self-contained.zip` | .NET 8 ウィンドウ |ランタイムを含む最新の Windows ビルドが必要な場合に使用します。 |
| `Virex.NET.Simulator-vX.Y.Z-net10.0-windows-win-x64-self-contained.zip` | .NET 10 ウィンドウ |サポートされている最新の Windows ターゲットを検証する必要がある場合に使用します。 |
...
|パッケージ |目的 |
| --- | --- |
| [Virex.NET.Contracts](https://github.com/vstechnology-tw/vst-virex.net.integration/releases/latest) |公開 C# データモデル、REST ルート、MQTT トピック名、TCP/NDJSON フォーマットおよび解析ツールを提供します。 |
