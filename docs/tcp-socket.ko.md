...
| --- | --- | --- | --- |
| `initialize` | `{"type":"initialize"}` | `Uninitialized` | `Initializing` 상태로 전환됩니다. 완료 후 `Ready` 상태의 `statusChanged`를 보냅니다. |
| `deinitialize` | `{"type":"deinitialize"}` | `Ready` | `Deinitializing` 상태로 전환됩니다. 완료 후 `Uninitialized` 상태의 `statusChanged`를 보냅니다. |
