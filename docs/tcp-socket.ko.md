...
| --- | --- | --- | --- |
| `initialize` | `type`가 포함된 명령 프레임 | `Uninitialized` | `Initializing` 상태로 전환됩니다. 완료 후 `Ready` 상태의 `statusChanged`를 보냅니다. |
| `deinitialize` | `type`가 포함된 명령 프레임 | `Ready` | `Deinitializing` 상태로 전환됩니다. 완료 후 `Uninitialized` 상태의 `statusChanged`를 보냅니다. |
