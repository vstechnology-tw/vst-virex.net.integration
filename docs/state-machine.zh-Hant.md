...
ProductInfo 完成順序與取消準備中的 Start，請參閱[2.2.3 命令完成與錯誤處理](communication-errors.zh-Hant.md)。

狀態機定義系統生命週期中，哪些命令在目前狀態下是合法的。用戶端送出命令前，應該以目前狀態作為判斷依據。
...
命令是用戶端或操作人員提出的要求。只有命令符合目前狀態規則時，系統才會接受。
