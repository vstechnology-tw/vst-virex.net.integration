...
`Virex.NET.Client` は、公開 REST、TCP、および MQTT 統合 API の型付きラッパーを提供します。
...
REST 通信の失敗と成功しない HTTP 応答は `VirexClientException` をスローします。プロトコルレベルのコマンド拒否は、`CommandResponse` で表されます。
