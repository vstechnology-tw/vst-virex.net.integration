...
| 資料模型 | 用途 |
| --- | --- |
| [ProductInfo](payloads/product/product-info.zh.md) | 執行與結果關聯使用的產品資訊。 |
| [SystemStatus](payloads/system/system-status.zh.md) | 目前生命週期狀態。 |
| [CommandResponse](payloads/commands/command-response.zh.md) | 命令接受或拒絕的結果。 |
| [ResultSummary](payloads/results/result-summary.zh.md) | 已完成執行的公開摘要資料。 |
| [ErrorInfo](payloads/system/error-info.zh.md) | 目前作用中的錯誤資訊。 |

這些是 JSON 資料模型。廠商可以使用 `Virex.NET.Contracts` 裡的 C# 類別，也可以在自己的語言裡定義等價模型。

## 參考文件

- [安裝 / 下載](installation.zh.md)
- [模擬器操作手冊](simulator.zh.md)
- [系統狀態機](state-machine.zh.md)
- [資料模型參考](payloads.zh.md)
- [REST API](rest-api.zh.md)
- [TCP Socket 通訊協定](tcp-socket.zh.md)
- [MQTT 事件](mqtt-events.zh.md)
- [範例程式](samples.zh.md)
