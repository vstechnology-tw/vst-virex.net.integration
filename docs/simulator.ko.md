...
| 3 | **State** | 현재 시뮬레이터 상태를 표시하고 주요 상호작용 버튼을 제공합니다. **Start Servers**는 REST, TCP, MQTT 엔드포인트를 엽니다. **Initialize**, **Deinitialize**, **Start Cycle**, **Stop**은 외부 REST 클라이언트가 호출할 수 있는 것과 동일한 공개 상태 전이를 실행합니다. |
| 4 | **Event Log** | 로컬 시뮬레이터 활동, 서버 시작/중지 메시지, 명령 거부 및 기타 진단 출력을 표시합니다. 버튼 동작이나 클라이언트 명령이 시뮬레이터에 도달했는지 확인할 때 사용합니다. |
...
5. 시스템을 초기화합니다.
6. 제품 정보를 보냅니다.
7. **Start Cycle**을 누르거나 동등한 클라이언트 명령을 보냅니다.
...
| **Start Cycle** | 실행을 시작합니다. `Ready`에서만 유효합니다. 응답 상태는 `Running`입니다. |
